...
**Current state:** Weeks 1–4 of 6 shipped. The harness is end-to-end runnable — `kaggle-slayer <workspace> --target <col>` invokes Gemini, the agent writes `agent/fe.py`/`agent/model.py`, calls `train_cv` and `submit_local`, and (gated) `submit_kaggle`. Real-Gemini acceptance gate is green on gemini-2.5-flash (~$0.001–0.01 per synthetic-comp solve). 316 non-slow tests, 8 slow-tier (opt-in).
...
│   ├─ resume.py                   # rebuild_conversation from run_log
...
└─ agent/                          # LLM side
    ├─ solver.py                   # reason-act loop + SolverContext
    ├─ llm_client.py               # GeminiClient (Content/Part + function_call/response)
    ...
    │   ├─ ml.py                   # set_cv, train_cv, submit_local, done, set_metric, submit_kaggle, request_human_approval
    │   └─ python.py               # run_python (sandboxed escape hatch)
    └─ prompts/system.md           # Solver system prompt

tests/
├─ unit/                           # ~300 unit tests
├─ integration/                    # fake-LLM + real-Gemini (slow tier)
...
- **Week 5** — telemetry surfacing (cost-ledger dashboard, OpenTelemetry traces, CV↔LB calibration tracker), chaos-tier tests (random tool-call failures at 5%), redesigned Streamlit dashboard.
- **Week 6** — three live Kaggle Playground comps + docs (`docs/architecture.md`, `docs/adr/*.md`, `.claude/commands/*.md`, `.claude/agents/harness-reviewer.md`).
